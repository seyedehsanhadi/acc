...
- AMPS no longer records a probe-collapsed 0 as a current-limit node's original value, which let a restore replay 0 and leave the port taking no input current with nothing reporting a fault. The charger-recovery path snapshots before it writes 0, charger speed is read after the restore instead of off a port the probes left capped, an implausible battery current is called stale rather than blamed on the cable, and the collapsed-contract verdict is HVDCP-only.
- AMPS resume states are honest: an unknown state used to read as `ok`, and a STUCK could be masked by a later OK. A discarded finalist no longer survives in the registry, the next-best pick cannot come back empty, a THROTTLE hold no longer runs a resume check that does not apply to it, and charger detection accepts a `usb` parent.
- Multi-node picks are stressed as a group, a discarded finalist is dropped from every list and the next-best promoted, and thermal-level nodes are discarded before the hammer rather than after a cool run reads clean. A finalist that was never verified downgrades the recommendation to needs-test.
- The AMPS voltage-cap probe is gone: writing `battery/voltage_max` is not reversible enough to be a candidate. Its deny lists are wider, and every copy of the engine prints a content id beside its version, so a stale copy cannot hide behind a truthful `V=` line.
