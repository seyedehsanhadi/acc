...
Fixes the "disabled the current limit but it still sticks" report: turning Charging power control off in AccA (or clearing max_charging_current any other way) cleaned the config and the UI, but the phone stayed capped at the old milliamp limit until a reboot. Two gaps stacked up. The restore path taken while the battery is NOT charging cleared the config and its own bookkeeping but never wrote the control nodes back, and every later restore attempt short-circuited on that missing bookkeeping. And AccA's fast set path only rewrites the config - the daemon owns the actual node release - but all of the daemon's release points sat in the charging/cooldown branches, so a phone resting at its pause limit (exactly when people flip the toggle) never released anything. Clearing the limit now writes the stored per-node defaults immediately in every state, and the daemon runs the same guarded release from its resting branch too. On top of that, the stored "defaults" are snapshots from probe time - a limit first applied on a 500 mA computer port would restore a 500 mA input ceiling onto a 2 A wall charger - so both restore paths now re-kick USB source detection afterwards and the charger renegotiates to its real capability. Verified on the Mi A3 on a 2 A wall charger: cap, pause at the limit, clear, and within one daemon tick the nodes return to defaults and charging climbs back to the full ~1.5 A.

AMPS (the charge-switch finder) gains a finalist stress-test: the switch that wins the recommendation is no longer trusted on its short passive window alone. An intermittently re-arming firmware node can hold through the normal probes and get crowned the top pick - the Mi A3's charge_control_limit passes as a clean bypass on a cool run, then the PMI632 quietly resets it minutes later. After the scan AMPS re-hammers the winning pick's stop value for a few seconds and demotes it if the firmware keeps overriding the writes or the pack keeps charging while engaged; and independently of that hammer, a winner whose _max sibling is a tiny level count (like charge_control_limit_max=6) is recognized as the kernel's thermal charge-control interface - firmware-owned levels, not a switch - and demoted even when the hammer happened to read clean on a cool phone. On a demote the recommendation is re-picked from the remaining finalists and the replacement is stress-tested too; a pick whose resolved control file does not match its own name is skipped rather than hammered, so the check can never punish the wrong switch. It touches only the winning node, snapshots and restores it, and can only demote (worst case = the second-best switch, never a regression); it runs even when the current sensor lies (every signal it uses is sensor-independent) and adds a few seconds only when something actually wins. Also fixes the frozen scan log in AccA: a leftover internal watchdog kept the output stream open for about two minutes after "restored" was printed, so the app appeared hung after every finished scan - the stream now closes within a second. Verified end-to-end on the Mi A3 across quick, deep and highest-accuracy runs: charge_control_limit wins as bypass, hammer reads clean cool, the thermal-level check demotes it, the re-pick lands on input_suspend and confirms it under the same hammer.

**v2025.5.18-6.5.1-rc5 (202505285)**
Fixes a charging-stuck bug reachable from AccA's "Test battery idle mode" button. That test runs a full charging-switch scan: it stops the daemon and briefly cuts charging to probe each candidate switch, and AccA wraps the whole thing in a timeout. On a phone where the scan runs longer than that timeout - a slow multi-switch device like the Mi A3 - the timeout's kill signal ended the test before it could clean up. The test restored itself only on a normal exit, never on that signal, so a killed test left its input-suspend cut still engaged and the daemon still stopped: charging stayed off until a reboot, and on some chargers (Snapdragon PMI632) that lingering cut escalates into a watchdog loop that only a reboot clears. The test's cleanup now runs on the kill signal too, guarded so it can never run twice and start two daemons, so an interrupted idle test always restarts the daemon and leaves no cut behind. Separately, the resume watchdog - which re-kicks USB input negotiation when charging fails to restart at your resume level - now also clears the whole charge-disable node family (input_suspend and its siblings), so a stray cut left by any path is released the instant the daemon wants to charge instead of waiting for the next reboot. Also fixes a 30-second freeze when a front-end lists charging switches (`acc -s s:`): if the candidate file did not exist yet the internal `sort` failed, and the failure tripped acc's crash-time log exporter, which spent ~30s taring every log into a .tgz. Listing switches on a missing file is now a clean empty result. Biggest change: the limit is now anchored to the FUEL GAUGE, not the kernel status/current, which lie on chargers that override the switch (Mi A3 charge_control_limit, which the PMI632 keeps re-arming). The status node flips "Charging" while the cell is actually held, and reads a charge current while a stray input cut blocks real charging - so the daemon could either let the battery drift past your limit or sit plugged-but-not-charging, and only a reboot fixed it. Enforcement now uses the coulomb-counted capacity, which cannot be fooled: if the cell sits above your pause level while a switch is engaged, a reversible hard input cut (on a node other than the switch's own) holds it until it drains back to the limit; and below your resume level the daemon releases any stray hard cut unconditionally, before it even consults the lying status, so a leftover input_suspend can never wedge charging. Both are reversible, release on unplug, and are a no-op on devices whose switch already holds. Reproduced and fixed on the Mi A3. No configuration changes.
