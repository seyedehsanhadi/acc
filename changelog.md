...
- Fixed (regression vs upstream): running `acc -s mcc=NNNN` (or setting a max current in AccA) printed a wall of "acc: can't create <node>: Permission denied" for the current-control nodes before the "Max charging current set" line. rc14 changed the write gate to skip the permission fix-up when a node "looks writable" - but as root every existing file looks writable (root bypasses the mode bits), so on a read-only charge node (no kernel write handler) ACC went straight to the write, which the kernel then refuses noisily. It now uses the permission fix-up itself as the writability test, exactly as upstream does: nodes the kernel won't accept are skipped silently instead of spamming a denial. Any residual refusal is quiet too. The rc14 fast-charge fix (only writing a node when its value truly changed) is unchanged. (Redmi/Derpfest field report; upstream VR-25 was unaffected, which pinpointed it.)
- Fixed: the Stop button in AccA (and any front-end "stop the service") could silently do nothing - the daemon kept running and you had to stop it from a terminal (Redmi Note 9S field report). ACC detected "is the daemon running" through a lock file, but AccA runs commands through one long-lived root shell, and an inherited handle on that lock could make the check wrongly report "not running", so the kill step was skipped and Stop no-op'd. Stop now also terminates the daemon by its exact process, independent of the lock state, so it always stops - with a normal signal first (so the on-exit cleanup runs and native charging is restored per the rule below), then a forced kill only for a stubborn survivor. Terminal `acc -D stop` was unaffected and still works.
- Fixed: stopping or restarting ACC while the battery was AT or ABOVE your limit briefly re-enabled charging, letting it creep past the cap until the next daemon started (a small overshoot on every restart). On exit at/above the limit ACC now leaves the pause held (the safe direction - it never overshoots); below the limit it resumes charging exactly as before. So after a stop, if you are already at your limit the phone stays paused until you replug.
- Added: a compact flight-recorder log (one state sample per daemon loop, ring-buffered) to make intermittent "it did X once" reports diagnosable after the fact.
- Improved: rate-limited notifications now survive daemon restarts and reboots, and the self-heal / resume-warning markers are cleared cleanly on every (re)start so a stale one can't suppress a real later warning.
- Fixed: on many fast-charging phones, charging dropped to Slow/Normal and never went Fast again after ACC (even "charge once to X%" could not bring it back). The daemon re-asserted charge-control nodes on a loop, re-writing the same value (plus a chmod and a retry burst). On phones that fast-charge through a charge pump / PPS / PD / VOOC path, any write to a charge node makes the charger re-run its input negotiation (AICL/APSD) and drop to the slow main charger; doing it repeatedly meant fast charge could never re-engage. ACC is now read-before-write everywhere (idempotent): a charge node is written ONLY when its value actually needs to change, and chmod runs only when needed. While charging healthily it touches nothing, so the negotiation is never disturbed; it still re-arms instantly the moment the firmware or a stray cut moves a node off target. Pause-at-limit behaviour is unchanged (a value that must change is still written; unit-verified on and off device).
- Fixed (Pixel/Tensor): fast charge and WIRELESS charging could stall with ACC running - wireless "hangs at charging", or the phone stays "Charging" after you lift it off the pad. On google,charger devices ACC drives the firmware limit by writing charge_start_level and charge_stop_level; it re-wrote both (with a chmod) on every daemon loop even when unchanged, and each write re-triggers the google charge state machine that also gates the wireless (p9221) path. This raw-echo path is now read-before-write too, so the levels are synced only on a real change. NOTE: if the phone's own Adaptive charging is enabled it writes the same nodes, so ACC still has to re-sync against it every loop - for the cleanest result turn Adaptive charging OFF (Settings > Battery). Field verification on Pixel 9a is in progress.
- Fixed: a batch of correctness fixes from a two-session line-by-line audit, each reproduced and verified on-device (mksh). `acc -D restart` now stops the running daemon before starting the new one (it previously left the old daemon running with stale config). A transient empty `current_now` read can no longer abort the daemon's charging-limit loop (coerced like the other sensor reads). The temperature-throttle path (`siop_level` / `charge_control_limit_max`) is now read-before-write like the rest, so it cannot re-trigger charger negotiation while warm (the one fast-charge path the idempotency above did not cover). A malformed or truncated config can no longer abort the daemon under `set -u` (`capacity`/`resetBattStats` indices default safely). A non-numeric max-current is rejected cleanly instead of erroring every daemon tick. `acc -H` no longer divides by zero at 0% battery. The crash-log auto-export now triggers on the intended exit codes (127/10) instead of never. A scheduled `at` command that merely starts with the letters "acc" (e.g. `accountsync`) is no longer rewritten to `/dev/acc...`. Config values with commas are no longer truncated at the first comma, and a lone `-` boot OFF-value is skipped instead of written to a switch node.

**v2025.5.18-6.5.1-rc13 (202505293)**

- Fixed: a phone could sit plugged in and keep draining, with `acca -i` showing "Draining" and a negative current, until ACC was restarted by hand (Redmi Note 9S field report). Two independent root causes, both closed. (1) A charge switch left OFF by a test that was force-killed mid-run (Android's phantom-process killer skips the cleanup traps) was an ENABLE-class node the daemon never re-checked; below your resume level ACC now writes the whole enable family back ON alongside the existing cut-clear, so charging revives on its own. This branch only runs below the resume level, so a pause is never broken. (2) On dual-path PMICs the battery-current SIGN flips with the charge mode (this device reads positive on the 5V trickle path and negative on the 9V parallel path, both genuinely charging), so any single learned polarity mislabeled one mode. ACC now decides charge vs discharge from the fuel gauge's `charge_counter` slope, which has no sign convention: a rising counter is charging and a falling one is draining regardless of the current sign. A 1% fuel-gauge dip on a slow charge can no longer teach an inverted polarity (the plugged learn now needs a real 2% move), and a polarity that flips twice is latched "unstable" so classification then comes from the coulomb slope and the kernel status word. `acca --state` gains a `ccDir` field. Everywhere the counter is missing or static it falls back to the previous sign logic, so nothing regresses (verified on a device with a frozen counter).
- Fixed: charging could fail to recover after uninstalling ACC. The generic un-cap sweep missed voltage_max nodes with no `*_design` sibling, nodes outside `/sys/class/power_supply` (such as `/sys/class/qcom-battery/restrict_cur`), and node names not in its list. Uninstall now replays ACC's own recorded stock values from the config first (every node it ever capped, wherever it lives), with the qcom-battery restrict family lifted as a generic fallback.
- Fixed: `acc uninstall` aborted with a shell syntax error on devices whose `/system/bin/sh` is mksh (most of them); the new config-restore parser used a `${var#*=(}` form only bash accepts. Rewritten with sed; caught by an on-device syntax check.
- Docs: KernelSU shows a freshly flashed module greyed with an "Updated" label until you reboot, which is the normal success state, not a failure. If the manager's flash button will not take the zip, open it from a file explorer with "Open with > KernelSU". Noted in the installer output and README, with a SuSFS note (it can hide `/data/adb/modules` from terminal shells).
