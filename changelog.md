...
AMPS (the charge-switch finder) only; no daemon or config changes. Adds a finalist stress-test. A native %-limit that passed L5's short enforcement window can still be an intermittently re-arming thermal node. The Mi A3's charge_control_limit accepts a percentage but the PMI632 quietly resets it minutes later, so the tester could crown it the top pick when it does not actually hold. After the scan AMPS now re-hammers only that one level finalist for a few seconds; if the firmware keeps overriding the written value or the pack keeps charging, the limit is demoted and a hard cut or bypass is recommended instead. It touches only the single level candidate, snapshots and restores that node, and can only demote (so the worst case is the second-best switch, never a regression), and it is skipped entirely on phones with no current sensor. Reproduced and verified on the Mi A3.
