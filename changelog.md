**v2025.5.18-6.5.1-rc6 (pending tag)**
AMPS (the charge-switch finder) only; no daemon or config changes. Adds a finalist stress-test: the switch that wins the recommendation is no longer trusted on its short passive window alone. An intermittently re-arming firmware node can hold through the normal probes and get crowned the top pick - the Mi A3's charge_control_limit passes as a clean bypass on a cool run, then the PMI632 quietly resets it minutes later. After the scan AMPS now re-hammers the winning pick's stop value for a few seconds and demotes it if the firmware keeps overriding the writes or the pack keeps charging while engaged; and independently of that hammer, a winner whose _max sibling is a tiny level count (like charge_control_limit_max=6) is recognized as the kernel's thermal charge-control interface - firmware-owned levels, not a switch - and demoted even when the hammer happened to read clean on a cool phone. On a demote the recommendation is re-picked from the remaining finalists and the replacement is stress-tested too. It touches only the winning node, snapshots and restores it, and can only demote (so the worst case is the second-best switch, never a regression); it is skipped on phones with no current sensor and adds a few seconds only when something actually wins. Verified end-to-end on the Mi A3: charge_control_limit wins as bypass, hammer reads clean cool, the thermal-level check demotes it, the re-pick lands on input_suspend and confirms it under the same hammer.
