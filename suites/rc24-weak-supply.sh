...
# The markers must be clean before the plug, or nothing below means anything. They are cleared by
# the daemon's next loop, and unplugged that loop is a long nap -- measured 123-197s on a Pixel 6a.
# Grading the instant the suite starts races the daemon and fails a phone that is behaving.
_mw=0
while [ $_mw -lt 210 ]; do
  _left=0
  for _m in .hvcontract .hvpeak .hvkicked .hvzero; do [ -e "$TD/$_m" ] && _left=1; done
  [ "$_left" = 0 ] && break
  sleep 10; _mw=$((_mw+10))
done
[ "$_mw" -gt 0 ] && echo "  ....  waited ${_mw}s for the daemon idle loop to clear the markers"
for _m in .hvcontract .hvpeak .hvkicked .hvzero; do
  [ -e "$TD/$_m" ] && no "$_m survived ${_mw}s unplugged, past a full idle loop" || ok "$_m absent before the plug"
done
